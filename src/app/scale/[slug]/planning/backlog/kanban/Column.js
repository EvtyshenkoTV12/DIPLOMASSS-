                      ...
                      <div className="p-2 text-sm overflow-hidden shadow-sm rounded-lg text-[13px] min-w-[250px]">
                        ...
                        {sprintDetails?.duration && (
                          <div className="flex mb-1 mt-1 text-[13px]">
                            <div className="font-semibold min-w-[75px]">Тривалість: </div> {sprintDetails?.duration}<br />
                          </div>
                        )}
                        {sprintDetails?.startDate && (
                          <div className="flex mb-1 mt-1 text-[13px]">
                            <span className="font-semibold min-w-[75px]">Дата початку: </span> {format(new Date(sprintDetails?.startDate), dateFormat)}<br />
                          </div>
                        )}
                        {sprintDetails?.endDate && (
                          <div className="flex mb-1 mt-1 text-[13px]">
                            <span className="font-semibold min-w-[75px]">Дата закінчення: </span> {format(new Date(sprintDetails?.endDate), dateFormat)}<br />
                          </div>
                        )}
                        {sprintDetails?.sprintStatus && (
                          <div className="flex mb-1 mt-1 text-[13px]">
                            <span className="font-semibold min-w-[75px]">Статус: </span> {sprintDetails?.sprintStatus}<br />
                          </div>
                        )}
                        {sprintDetails?.sprintGoal && (
                          <div className="flex mb-1 mt-1 text-[13px]">
                            <span className="font-semibold min-w-[75px]">Мета: </span> {sprintDetails?.sprintGoal}<br />
                          </div>
                        )}
                      </div>
                    </Popover>
